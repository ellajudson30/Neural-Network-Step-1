...
import csv
from s2_rk45utils import run_RK45
import time
...
with open('errors.csv', mode='r', newline='', encoding='utf-8') as file:
    # Create a reader object
    csv_reader = csv.reader(file)
...
for fold, (train_index, val_index) in enumerate(kfold.split(Training_data)):
    ...
    batch = 16
    Train_loader = DataLoader(TrainData, batch_size=batch, shuffle=True)
    Val_loader = DataLoader(ValData, batch_size=batch, shuffle=True)
...
Test_loader = DataLoader(Testing_data, batch_size=batch, shuffle=True)
